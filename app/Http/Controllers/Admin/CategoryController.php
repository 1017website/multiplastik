...
class CategoryController extends Controller
{
    ...
    {
        $q = Category::with(['brand', 'masterCategory'])->withCount('products');
        if ($request->filled('brand_id')) {
            $q->where('brand_id', $request->brand_id);
        }
        if ($request->filled('master_category_id')) {
            $q->where('master_category_id', $request->master_category_id);
        }
        $categories = $q->orderBy('master_category_id')->orderBy('brand_id')->orderBy('sort_order')->paginate(20)->withQueryString();
        $brands = Brand::orderBy('name')->get();
        $masterCategories = MasterCategory::orderBy('sort_order')->orderBy('name')->get();

        return view('admin.categories.index', compact('categories', 'brands', 'masterCategories'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.categories.form', ['category' => new Category, 'brands' => $brands, 'masterCategories' => $masterCategories]);
    }
    ...
    public function edit(Category $category)
    {
        ...
        return view('admin.categories.form', compact('category', 'brands', 'masterCategories'));
    }
