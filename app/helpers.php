...
if (! function_exists('setting_enabled')) {
    /**
     * Baca site setting bertipe aktif/nonaktif.
     */
    function setting_enabled(string $key, bool $default = true): bool
    {
...
if (! function_exists('slide_link')) {
    ...
    function slide_link(?string $link): string
    {
        ...
        if ($link === 'nav:brands') {
            return route('site.brands');
        }
        if (str_starts_with($link, 'nav:brand:')) {
            return route('site.brand', substr($link, 10));
        }
