...
@section('content')
    ...
    <form class="card p-3 mb-3" method="GET">
        <div class="row g-2 align-items-end">
            ...
            @if (request()->hasAny(['category_id', 'search', 'per_page']))
                <div class="col-md-1">
                    <a href="{{ route('admin.products.index') }}" class="btn btn-outline-secondary w-100" title="Reset"><i
                ...
                <tbody>
                    @forelse($products as $p)
                        <tr>
                            ...
                            <td><small
                                    class="text-muted">{{ $p->category->brand->name }}<br>{{ $p->category->name }}</small>
                            </td>
                            <td><strong>{{ $p->name }}</strong><br><code class="small">{{ $p->slug }}</code>
                            </td>
                            <td><small>{{ is_array($p->specs) ? count($p->specs) : 0 }} item</small></td>
                            <td>{{ $p->sort_order }}</td>
                            <td>
                                @if ($p->is_active)
                                <span class="badge-soft success">Aktif</span>@else<span
                                        class="badge-soft danger">Nonaktif</span>
                                @endif
                            </td>
                            <td>
                                <a href="{{ route('admin.products.edit', $p) }}" class="btn btn-sm btn-outline-primary"><i
