...
@php
    // resolve link tombol slide: nav:brands / nav:brand:slug / wa / url
...
<section id="hero-s">
  <div class="slider-track" id="sliderTrack" style="width:{{ max(count($slides),1) * 100 }}%;">
    @foreach($slides as $slide)
      <div class="slide" style="width:{{ 100 / max(count($slides),1) }}%;">
        <div class="slide-bg">@if($slide->background_image)<img src="{{ $slide->background_url }}" alt="" loading="{{ $loop->first ? 'eager' : 'lazy' }}"/>@endif</div>
        <div class="slide-color" style="background:linear-gradient(105deg,rgba(17,17,17,.88) 40%,rgba(192,39,45,.3) 75%,rgba(17,17,17,.1) 100%);"></div>
        <div class="slide-accent"></div>
        <div class="slide-content">
          @if($slide->tag)<div class="slide-tag">{{ $slide->tag }}</div>@endif
          <h2 class="slide-title">{{ $slide->title_top }}@if($slide->title_em)<br><em>{{ $slide->title_em }}</em>@endif@if($slide->title_bottom)<br>{{ $slide->title_bottom }}@endif</h2>
          @if($slide->subtitle)<p class="slide-sub">{{ $slide->subtitle }}</p>@endif
          <div class="slide-btns">
            @if($slide->btn_primary_text)
              <a href="{{ slideLink($slide->btn_primary_link) }}" class="btn-p" @if(str_starts_with($slide->btn_primary_link ?? '','http') || $slide->btn_primary_link==='wa') target="_blank" @endif>
                @if($slide->btn_primary_icon)<i class="{{ $slide->btn_primary_icon }}"></i>@endif {{ $slide->btn_primary_text }}
              </a>
            @endif
            @if($slide->btn_secondary_text)
              <a href="{{ slideLink($slide->btn_secondary_link) }}" class="btn-o" @if(str_starts_with($slide->btn_secondary_link ?? '','http') || $slide->btn_secondary_link==='wa') target="_blank" @endif>
                @if($slide->btn_secondary_icon)<i class="{{ $slide->btn_secondary_icon }}"></i>@endif {{ $slide->btn_secondary_text }}
              </a>
            @endif
          </div>
        </div>
  ...
  <div class="hero-stats">
    <div class="hstat"><div class="hstat-n">{{ setting('stat_1_number') }}</div><div class="hstat-l">{{ setting('stat_1_label') }}</div></div>
    <div class="hstat"><div class="hstat-n">{{ setting('stat_2_number') }}</div><div class="hstat-l">{{ setting('stat_2_label') }}</div></div>
    <div class="hstat"><div class="hstat-n">{{ setting('stat_3_number') }}</div><div class="hstat-l">{{ setting('stat_3_label') }}</div></div>
    <div class="hstat"><div class="hstat-n">{{ setting('stat_4_number') }}</div><div class="hstat-l">{{ setting('stat_4_label') }}</div></div>
  </div>
</section>
...
<div class="promo-bar">
  <div class="promo-track">
    @foreach(array_merge($promos, $promos) as $p)
      <span class="promo-item"><i class="fas fa-star" style="font-size:9px;margin-right:8px;opacity:.7;"></i>{{ $p }}</span>
    @endforeach
  </div>
</div>
@endif
  ...
  <div class="home-brands">
    @foreach($brands as $b)
      <a href="{{ route('site.brand', $b->slug) }}" class="home-brand-card" style="text-decoration:none;">
        @if($b->logo)<img src="{{ $b->logo_url }}" alt="{{ $b->name }}"/>@endif
        <h3>{{ $b->name }}</h3>
        <p>{{ $b->tagline }}</p>
...
<section id="about-s">
  <div class="about-grid">
    <div class="about-imgwrap fu">
      @if(setting('about_image'))<img src="{{ media_url(setting('about_image')) }}" alt="Tentang Kami"/>@endif
      <div class="about-badge"><strong>{{ setting('about_badge_number') }}</strong><span>{{ setting('about_badge_text') }}</span></div>
    </div>
    <div class="fu">
        ...
        @for($i = 1; $i <= 4; $i++)
          @if(setting("about_feat_{$i}_title"))
            <div class="about-feat">
              ...
              <div class="about-feat-text"><strong>{{ setting("about_feat_{$i}_title") }}</strong><span>{{ setting("about_feat_{$i}_text") }}</span></div>
            </div>
          @endif
        @endfor
      </div>
...
<!-- ==================== KONTAK ==================== -->
<section id="kontak-s">
  <div class="fu"><div class="sec-label">Hubungi Kami</div><h2 class="sec-title">Lokasi & Kontak</h2><div class="sec-div"></div></div>
  <div class="kontak-grid fu">
    <div>
      <div class="ki"><div class="ki-icon"><i class="fas fa-map-marker-alt"></i></div><div class="ki-text"><strong>Alamat</strong><span>{{ setting('contact_address') }}</span></div></div>
      <div class="ki"><div class="ki-icon"><i class="fab fa-whatsapp"></i></div><div class="ki-text"><strong>WhatsApp</strong><a href="{{ wa_link() }}" target="_blank">{{ setting('contact_whatsapp_display') }}</a></div></div>
      <div class="ki"><div class="ki-icon"><i class="fas fa-envelope"></i></div><div class="ki-text"><strong>Email</strong><a href="mailto:{{ setting('contact_email') }}">{{ setting('contact_email') }}</a></div></div>
      <div class="ki"><div class="ki-icon"><i class="fas fa-clock"></i></div><div class="ki-text"><strong>Jam Operasional</strong><span>{{ setting('contact_hours') }}</span></div></div>
    </div>
    @if(setting('contact_map_embed'))
    <div class="map-wrap">
      <iframe src="{{ setting('contact_map_embed') }}" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
    </div>
    @endif
  </div>
</section>
...
<script>
  ...
  function goSlide(i){ curSlide = (i + SLIDES_COUNT) % SLIDES_COUNT; renderSlide(); resetTimer(); }
  function sliderMove(dir){ goSlide(curSlide + dir); }
  function resetTimer(){
    clearInterval(sliderTimer);
    if (SLIDES_COUNT > 1) sliderTimer = setInterval(() => sliderMove(1), 5500);
  }
  renderSlide(); resetTimer();
